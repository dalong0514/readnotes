...
在前面两章中，我从阐述概念开始，论述了行动动机如何导致了某些类型的互动，如何导致了社会资本的效用。我认为行动导致了位置、权

威、规则和代理关系不断复杂的社会结构的生成（第八章）。本章通过考察

在一个复杂的社会结构（组织）背景中社会资本的获取和使用，继续扩展这

条论述线索。这里我先假定等级制结构（如组织）是稳定的和能够正常运行的，然后确定行动者如何通过他们的位置获取好的社会资本 一一 嵌入在其他位置中的，特别是更高的等级制位置中的资源。因此，这里首先论述的是结构性限制，其次说眀获取社会资本的行动如何在这些限制下展开。
