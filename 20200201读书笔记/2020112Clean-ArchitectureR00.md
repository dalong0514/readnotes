...
感觉渐渐拨开迷雾了。首先，A 依赖于 B，那么 A 就是 B 的「插件」，随插随用。要让数据库模块和交互界面模块依赖于业务模块，那么前两者可以作为业务模块的插件。A 依赖于 B，即要 import 引用 A，对应于第 5 章中图 5.1 里，Main 函数是 A，下面其调用的 HL1 函数是 B，HL1 函数可能是其他模块里的功能函数，要引用进来的。A 类继承于 B 类，说明 A 依赖于 B。
