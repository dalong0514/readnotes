...
1、Scrapy 是一个健壮的抓取网络资源的框架，它基于事件的架构，可以让我们进行串联操作，清洗、形成丰富数据，或存入数据库等等，同时不会有太大的性能损耗。

2、网页的 HTML、HTML 的树结构和用来筛选网页信息的 XPath。创建可靠的 XPath 表达式从 HTML 抓取信息。

5、快速构建爬虫，介绍更多关于爬虫的特点，模拟登陆、更快抓取、使用 APIs、爬 URL 的方法。
