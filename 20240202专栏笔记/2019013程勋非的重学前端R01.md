...
介绍了 JavaScript 运行时的类型系统，除了这七种基本语言类型，还有一些语言的实现者更关心的规范类型。程序 = 算法 + 数据结构，运行时类型包含了所有 JavaScript 执行时所需要的数据结构的定义，所以我们要对它格外重视。
...
「基于类」并非面向对象的唯一形态，基于原型也是一种形态，而且是一种非常优秀的抽象对象的形式。

0104JavaScript对象你知道全部的对象分类吗.md

介绍了一些不那么常规的对象，并且还介绍了 JavaScript 中用对象来模拟函数和构造器的机制。这是一些不那么有规律、不那么优雅的知识，而 JavaScript 正是通过这些对象，提供了很多基础的能力。

0105JavaScript执行1Promise里的代码为什么比setTimeout先执行.md

宿主发起的任务称为宏观任务，JavaScript 引擎发起的任务称为微观任务。许多的微观任务的队列组成了宏观任务。

0106JavaScript执行2闭包和执行上下文到底是怎么回事.md

编程语言里闭包的概念；各个版本中的 JavaScript 标准中的概念：执行上下文、作用域、this 值等等。

0107JavaScript执行3你知道现在有多少种函数吗.md

认识了 ES2018 中规定的各种函数，简单介绍了 8 种函数。围绕 this 这个中心，介绍了函数的执行上下文切换机制。同时我们还讲解了 this 中的一些相关知识。包括了操作 this 的内置函数。

0108JavaScript执行4try里面放return-finally还会执行吗.md

JavaScript 语句执行的原理。因为 JavaScript 语句存在着嵌套关系，所以执行过程实际上主要在一个树形结构上进行，树形结构的每一个节点执行后产生 Completion Record，根据语句的结构和 Completion Record，JavaScript 实现了各种分支和跳出逻辑。
...
3『 utf 编码方式，规定了码点在计算机中的表示方法。换个说法是，字符集映射成机器数据流的规则。Unicode 字符集有 16 大块，而 BMP 是第一块基本的字符区域；编码的有关概念：对于英语我们一般不会遇到什么编码问题，因为一个英文字符在系统里占一个位置。对于中文的话，微软设置了一个编码 gbk，一个汉字在系统里占有 2 个位置，gbk 编码虽然节省空间，但只包含有常用字。对于那些生僻的字，就用 utf-8 编码，这个编码一个汉字是占有 3 个位置。摘自「2019004尹会生的零基础学Python/1501爬虫.md」』
...
1『为啥说 Symbol 的出现可以暴露出许多内置接口，目前无法理解。（2020-08-27）』
...
1『牢记 ES6 新增了这 3 个内置函数：获取一个对象的原型、设置一个对象的原型以及根据指定的原型创建对象。上面的 3 个原型方法好好用，感觉可以掌控原型的一切，开发新功能以后别用 new 构造函数来构建对象了，哈哈。（2020-05-28）』
...
1『箭头函数没有 [[construct]] 私有属性，无法作为构造器来使用。』
...
1『这里实现「私有属性」的知识点很重要，「JS 精粹」里也有提到，但目前还没吃透。（2020-05-29）回复：现在算是吃透了，属性 a 在外面是无法直接访问的，比如通过 o.a 语句，只有特通构造对象返回对象里的一个属性（这里类似于 Java 里成员函数），才能访问到属性 a，而这个过程又用到了闭包的知识，返回对象里的 this.a 可以访问到外层的属性 this.a。这种方式实现了面向对象的「封装」特性，确实精妙。（2020-08-28）』
...
1『只有执行了 resolve() 方法，then() 里的函数才会调用。』
...
1『 async 异步函数，其底层运行基础是 Promise，可以说是对 Promise 的一种封装；async 异步函数必定返回 Promise，在 async 异步函数中可以使用 await 来等待一个 Promise。』
...
1『感觉作者原文里的那张图逻辑不好，闭包的概念还是去看「2020007MediumR05」里讲解执行上下文 EC 的那篇文章。（2020-08-28）』
...
1『感觉 winter 没把 Realm 讲清楚，目前还是不清楚它的概念。（2020-08-28）』
...
箭头函数里，this 值都是指向全局变量。这个应用场景实在太重要了。

```js
getData() {
  var that = this;
  console.log(axios.get(this.dataurl));
  // axios 获取后端数据，关键点在 that
  axios.get(this.dataurl).then(function(response) {
    console.log(response);
    that.requestdata = response.data.data.monomerid;
    that.roomdata = response.data.data.roomdata;
    that.handleData(that.requestdata);
  })
  .catch(function(e) {
    console.log(e);
  });
...
getData() {
  console.log(axios.get(this.dataurl));
  // axios 获取后端数据，关键点在用箭头函数后 this 可以绑定到全局变量，否者使用 that
  axios.get(this.dataurl).then((response) => {
    console.log(response);
    console.log(this.requestdata);
    this.requestdata = response.data.data.monomerid;
    this.roomdata = response.data.data.roomdata;
    this.handleData(that.requestdata);
  })
  .catch(function(e) {
    console.log(e);
  });
...
上面代码是开发数据流时遇到的问题，关键点在用箭头函数后 this 可以绑定到外层词法环境的 this 值，这里即全局变量，否则得使用「var that = this」语句。实在太赞了，解决之前很大的一个困惑。（2020-05-19）
...
1『图里，栈里的函数执行上下文（基本等同与词法环境）中，有一个执行外层执行上下文的引用，靠它访问外层的执行上下文，这是闭包的底层原理。补充：结合之前看的那篇 Medium 讲 JS 栈的文章，这里还有个知识点：变量 b 的申明用了 var，会直接产生了一个「全局上下文」，这点有应用场景的。（2021-05-05）』
...
1『目前个人是觉得 winter 把 this 的知识点讲的好凌乱。（2020-08-28）』
...
1『原来任何 JS 语句前都可以加标签，标签后面直接跟着冒号。』
...
1『原来这个写法的官方名字叫「字符串模板语法」。（2020-06-02）』
...
1『不错不错，生成函数和迭代器的结合，让一个对象具备 for of 循环语句。』
...
1『对象、函数、类，外面加一个括号就可以变成直接量。这里 winter 说的直接量即之前很多地方看的字面量。』
