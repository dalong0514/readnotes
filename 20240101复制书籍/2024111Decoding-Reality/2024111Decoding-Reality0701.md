...
当然，事件之间的互信息并非总是绝对的，有时候也会存在不确定性。比如说，你的朋友喝饮料时，你「通常」会跟着喝，但并不是每次都会这样。在这种情况下，两个事件之间的相关性就会降低，推断的准确度也会相应下降。
...
我们能否将这个简单的概念应用到人类社会这样复杂的系统中呢？乍看之下，这似乎很难实现。我们可以把人类比作小磁铁（假设他们在「政治立场」上可以指向左或右），然后研究在什么条件下，整个社会会向左或向右产生倾向，以及这种转变是如何只需要很小的外部影响就能实现。这就像是把磁体的相变现象应用到社会领域。

你可能会说，与人类相比，原子是非常简单的系统。毕竟人类能够思考、感受和表达愤怒，而原子是没有生命的，其表现方式也要简单得多。但这不是重点！关键在于我们只关注人类（或原子）的某一个特定特征。原子本身也并非那么简单，只是我们选择只关注它的磁性特征，从而简化了问题。人类当然更为复杂，但此时我们只关心他们的政治倾向，而这一点在现实中往往是相对简单的。

我们来进一步探讨这个话题。人类和原子之间有一个根本的区别：人类拥有自由意志（free will）。人类可以自主行动，能够做出自己的决定，掌控自己的命运，而原子则做不到这些。尽管关于自由意志的讨论十分复杂（我们会在后面详细讨论），但在这里我们已经可以谈一些相关的观点。

设想你在观察一条繁忙街道的卫星图片，比如伦敦的牛津街。如果你使用 google.com 的卫星图片，你将无法分辨出每个行人的具体身份（当然，CIA 或 MOSSAD 应该能获得更高清晰度的图像）。在这些图片中，人们看起来就像是在两条直线（牛津街人行道的两边）之间移动的小点。街道中间还有一些体积较大的点，那是来往的车辆，不过我们暂时不去讨论这部分。

让我们来观察这些点的运动方式。它们主要是在街道上上下下移动，但由于需要相互避让，也会有一些左右移动。假设现在有人给你看这样一幅运动的点阵图，但没告诉你这些点代表着人。你能仅凭这些点的运动模式，判断出它们是代表有生命的物体（也许还是具有智能的生命？）还是无生命的物体（比如原子或碰撞的台球）吗？

很可能你无法做出判断，因为在这种情况下，人群和原子的运动模式惊人地相似（它们都会避免碰撞，只是方式不同：人是通过视觉感知来移动，而原子是通过静电斥力）。这两种现象都可以被描述为扩散过程（diffusion process)—— 整体上呈现出一个主要方向（上下）的流动，但时不时会出现一些随机的偏移。这正好说明了我想表达的观点：生命体和非生命体的行为模式可以表现出惊人的相似性。

让我们回到相变（phase transition）与人类社会的话题。想象一下，我们有这样一个系统：它像链条一样，每个部分都只能和左右相邻的两个部分产生相互作用。这个模型最早是由 Ernest Ising 在物理学领域提出的，他在 1920 年代读博士时研究过这个问题。然而结果令人遗憾，他通过严密的数学证明表明，这种模型中根本不可能出现相变现象。换句话说，如果把链条上的原子想象成小磁铁，不管外界条件如何变化，这些小磁铁都不可能自动排列整齐。这个结论对 Ising 来说是个巨大的打击，因为他原本希望能从微观层面解释相变现象，结果却让他在获得博士学位后就放弃了物理学研究！

不过对 Ising 来说真是太可惜了，因为大约 20 年后，Lars Onsager 在一篇极具创见的论文中证明，如果把这些原子排列成二维平面（而不是一维链条），那么在温度足够低的情况下，确实会发生相变。这项发现为 Onsager 赢得了化学领域的诺贝尔奖。从那以后，相变理论研究开始蓬勃发展，诞生了许多重大突破，并产生了多位诺贝尔奖得主（包括 Anderson 在内）。

在我所了解的相变（phase transitions）研究中，最引人注目的发现之一是一个非常普遍的「禁止定理」（no-go theorem）。禁止定理是一类重要的理论成果，它们可以用来证明某些特定的物理现象是不可能发生的。物理学家 Ising 提出的结果就是一个特殊的禁止定理，它证明了在一维系统（即类似于链状结构）中不可能发生相变。更有趣的是，科学家们还证明了在二维系统（即类似于平面结构）中也普遍不存在相变现象。在二维情况下，这个禁止定理只有一个例外，那就是物理学家 Onsager 所分析的模型（这使他的研究显得格外幸运）。这些发现具有深远的意义 —— 当我们在现实世界中观察到冰变成水、水变成水蒸气这样的相变现象时，这实际上已经从数学上证明了我们所处的宇宙必须至少是三维的。

那么，这一切与人类社会有什么联系呢？在上述系统中，我们可以把原子不是看作小磁铁，而是看作人类。诚然，如前所述，人类比原子要复杂得多，但我们的某些特征其实非常简单。让我们思考一下人类及其政治倾向。假设在一个社会中，我们只有两种选择：要么保守，要么自由。这两种状态就像磁铁的两种状态在人类社会中的对应：要么指向右边，要么指向左边（当然，我很清楚一些愤世嫉俗的人可能会说，在政治中「掌权」或「失势」才是更重要的方向）。

一般来说，一个社会中总是有一部分人持保守立场，另一部分人持自由立场。但是，如果我们问：在什么条件下所有成员都会变成自由派呢？根据我们对伊辛模型（Ising model）结果的讨论可以推断，如果我们有一个非常简单的社会，其中每个成员只与最近的两个邻居讨论政治（而不与其他人交流），那么就不可能出现一个完全由自由派构成的社会。换句话说，我们永远不可能实现自发的自由化，社会中总会存在一定数量的保守派。

一个一维结构的社会组织非常容易理解，因为它的变化都很可预测。电影《Goodfellas》（好家伙）中描绘的黑手党就是一个典型例子（这显然与自由社会大相径庭）。黑手党的最高领袖，也就是意大利语中所说的「教父中的教父」（capo di tutti capi），是一个叫 Pauly 的人。他只会向最信任的两个手下传达指令，很少与其他人直接接触。这两个手下再将命令传达给他们各自信任的下属，以此类推。这种结构不仅能有效地防止 FBI 发现黑手党的核心人物，同时也确保了组织内部的稳定性。正因如此，黑手党组织整体上不太可能发生剧烈的变革，比如突然更换领导人，或是因为个别叛徒告密而导致组织瓦解。

有趣的是，如果我们假设每个人都能与其他所有人产生互动，那么即使在一维空间中也可能发生状态的突变。而现实世界则介于完全隔离和完全互联这两种极端情况之间。我们既不会仅仅与邻居打交道，也不可能和世界上的每一个人都有联系。正如我之前提到的，我们能够保持经常联系的家人、朋友和熟人的数量通常在 200 人左右。

事实上，我们与他人的社交联系数量遵循一个称为幂律分布（power law distribution）的规律。认识很多人的人数要比认识较少人的人数要少得多。这种数量比例遵循着一个严格的规律，这与本书前面提到的描述字母出现频率的齐普夫定律类似。具体来说，拥有 1000 个社交联系的人比拥有 10 个社交联系的人要少 100 万倍。就像在英语中较长的单词出现频率较低一样，拥有大量社交连接的人在统计学上也是较为罕见的现象。

人与人之间接触频率的不平衡形成了一个非常重要的模型：人们与周围的人频繁接触，同时偶尔也会与远方的人产生联系。这就是所谓的「小世界网络」（Small World Network），它很好地解释了为什么疾病能在我们的世界中快速传播以及其传播方式。当我们患病时，疾病通常会先感染周围的人。而当这些人中只要有一个进行远途旅行，病毒就有可能在远距离的地方开始传播。这也正是我们对猪流感以及其他可能致命的病毒如此担忧的原因。

我们来探讨一个问题：为什么很多人认为（不管这种观点是否正确），信息革命对社会的改变将超过历史上任何一次革命，包括我们在前几章讨论过的工业革命。著名社会学家 Manuel Castells 就认为，互联网将为我们的社会带来前所未有的深刻变革。他的这一观点建立在相变（phase transitions）理论的基础之上，虽然作为一名社会学家，他对这一概念的理解可能与物理学家从数学角度的诠释有所不同。

为了解释这一点，我们可以把早期人类社会理解为具有很强的「地域性（local)」特征。当时的部落分散在不同的地方，彼此之间很少有往来和交流。即使到了十九世纪末期，思想的传播和信息的交流依然是一个缓慢的过程。也就是说，在很长一段时间里，人类社会的交流范围都受到了严重的地理限制。从物理学的角度来看，这种情况下就不可能发生突然的剧烈变化。考虑到社会系统的复杂性，我们更倾向于说社会的根本变革是「不太可能」而非「完全不可能」。而在最近这些年，随着科技的快速发展，我们不仅可以四处旅行，还能通过互联网与世界上几乎任何一个人进行学习和交流。

早期社会就像伊辛模型（Ising model）那样，个体之间的互动较为局限和固定，而后来的社会则更像小世界网络（small world networks）那样逐渐复杂化。然而，我们现在正在快速迈向一个新的阶段：每个人都能够并且实际上正在与世界上的任何其他人产生联系。正是在这种高度互联的状态下，社会系统发生突然性质变化（phase transitions）的可能性也在不断增加。在当今世界，资金（甚至劳动力）能够在几秒钟甚至更短的时间内从地球的一端快速流动到另一端。毫无疑问，这种前所未有的互联性正在深刻影响着我们社会的方方面面。
...
对于信息时代的影响，社会学家们意见不一。乐观派认为，信息时代将创造一个更加公平的社会，不仅能提高所有人的生活水平，还能缩小富人和穷人之间的差距。悲观派则认为，新时代将给现有社会秩序带来剧变，可能通过多种方式引发社会动荡，比如家庭结构解体导致犯罪增加、全球恐怖主义的威胁、全球气候变暖等问题。
