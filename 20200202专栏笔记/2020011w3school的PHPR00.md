...
1『关键是这些函数是给 web 开发定制的，比如生成图片、pdf 等。信息来自书籍「2020080PHP_and_MySQL_Web_」；小程序里函数语法与之一样，发现 PHP 的句话跟小程序的 js 句法是相通的。』
...
1『发现在 md 文件里可以显示基本的表单代码结果，进而发现 md 支持 HTML 格式。』
...
1『点完提交后浏览器的地址就直接转到了 welcome.php 文件上，即「http://192.168.64.2/2020011w3school/welcome.php」。这里注意，文件名一定要是 welcome.php，因为 action 的属性值 是它。』
