...
在完成管线图（包括控制仪表，即过程和仪表图（P&ID)）之前，无法进行 HAZOP。应在图纸完成后尽快进行分析，且要在详细设计开始之前完成。因此，可供实施的「时间窗口」是有限的，所以要提前计划好相关会议。等到管线图准备就绪后再期望团队成员能够立即参与是不现实的。
...
在许多情况下，生产过程中使用的物料意外进入了公用工程系统（如蒸汽、空气、氮气、水等），这通常是因为公用工程系统的压力比平常低，或者生产系统的压力比平常高。这种污染随后通过公用工程系统扩散到工厂的其他部分。有一次，乙烯通过泄漏的热交换器进入了蒸汽加热器。蒸汽管线的另一个分支为控制室地下室的空间加热器供应蒸汽，冷凝水则排放到建筑物内的开放式排水沟。结果，乙烯在地下室积聚，并被点燃（可能是由未做防护的电气设备引起），导致整个建筑物被摧毁。值得注意的是，如果事先进行危险与可操作性分析（HAZOP，Hazard and Operability Study），本可以发现乙烯泄漏的途径，从而避免这次事故。
...
对该系统进行的危险与可操作性分析（HAZOP）本应识别出可能发生的压力异常情况。这些异常会导致碳氢化合物（主要是石油和天然气的成分）通过滑动阀门倒流至静电除尘器（一种用于去除气体中颗粒物的设备），而在那里很可能发生点燃。分析还应该强调，需要有效的隔离措施来避免这种情况，而仅仅使用一个用于控制的滑动阀门很可能不足以达到这个目的。
...
HAZOP（危害与可操作性分析）团队中还有一个 Kletz 没有提到的关键成员，那就是记录者（Scribe）。这个角色通常由一名初级过程工程师担任。记录者的主要职责是记录 HAZOP 的输入和输出，但他们同时也是团队的正式成员，应该能够为技术讨论做出贡献。他们需要理解讨论的内容，能够快速记录笔记（如今通常直接使用电脑），并在必要时请求与会者放慢讨论速度或要求澄清。记录者通常还负责大部分准备工作，例如整理相关文档。有报告指出，「一个优秀的记录者可以减少 30% 的 HAZOP 完成时间，而一个不称职的记录者几乎可能导致整个分析无法进行。」
...
建议危险与可操作性分析（HAZOP）不应占用连续的整天时间，这一观点仍然是恰当和有效的。遗憾的是，这一指导原则经常被忽视，尤其是在大型项目中。问题在于，严格遵守这些限制将对项目进度产生重大影响。如果 HAZOP 导致项目交付延迟，它将很可能招致不满。这种不满可能会对人们对过程安全（指在工业过程中预防和控制可能导致事故的风险）重要性的认知产生负面影响，甚至可能适得其反。
...
前面描述的事故突出表明，如果能够有效利用来自多个来源的知识，HAZOP 分析会发挥最大效用。Kletz 在其著作中强调了许多不同的例子 [2]。第一个案例是位于防泄漏堤（bund）内的浮顶储罐，如图 1.3 所示。

图 1.3：考虑软管泄漏的可能性。

案例 1.25

储罐内装有油。雨水可以从屋顶排入防护堤，然后从那里流入主排水系统。假设一个危险与可操作性分析（HAZOP）团队正在评估是否有除水以外的其他物质可能进入主排水系统。要发生这种情况，需要同时满足三个条件：软管有破损、两个阀门都处于打开状态。缺乏经验的团队可能会认为这种三重巧合的概率极低，因此不值得进一步考虑。然而，熟悉工厂实际运营的人会意识到，在长时间降雨期间，操作员可能会违反操作规程，将两个排水阀都保持打开状态，以避免频繁查看和操作储罐。在这种情况下，如果软管上有任何破损，油就会污染主排水系统。

Kletz 使用以下例子来强调专业知识的重要性。

案例 1.26

一个容器中盛放着液态硫（熔点 120°C）。工程师们对流程图进行了危险与可操作性研究（HAZOP)；在考虑「压力增加」的情况时，团队认为已采取的防止排气管堵塞的预防措施（包括一个液封）是足够的。然而，在后来对详细管线图进行 HAZOP 分析时，当讨论到「温度升高」的情况，有人指出硫的粘度在 200°C 以上会急剧上升。虽然在正常操作中不会达到这个温度，但如果容器暴露在火中就可能达到。这时，液封中的硫可能会变得非常粘稠，以至于阻止容器的泄压。因此，工程师们不得不重新设计泄压系统。

另一个案例中，一个溶剂储罐通过一个密封罐进行排气。后来，工程师们添加了一个电加热器。虽然报告中没有说明原因，但很可能是为了防止在寒冷天气中溶剂冻结。对这一改动进行了 HAZOP 分析，但团队的所有成员都是化学工程师，没有电气工程师或设备供应商代表参与。结果，这些化学工程师都没有意识到一个潜在的危险：如果密封罐中的液位丢失，加热器的温度可能会升高到超过溶剂的自燃点。

Kletz 在其研究中强调了本地知识对于识别潜在危险的重要性 [2]。

案例 1.27

在某些情况下，即使在设备停机（shutdown）期间没有高压气体可用时，也需要操作阀门。维护团队成员被要求操作这些阀门，但却没有得到具体的操作指导。于是，他们采用了水和液压泵来完成操作。令人惊讶的是，包括操作工代表在内的危险与可操作性研究（HAZOP，Hazard and Operability Study）团队成员竟然都不知道阀门是以这种方式被操作的。

这个案例说明，HAZOP 的有效性在很大程度上取决于参与人员的知识和经验。如果他们对实际操作情况不了解，HAZOP 就无法有效识别和评估潜在的危险。

案例 1.28

某工厂安装了一批由高压气体控制的吹除阀 (blowdown valves)。一个寒冷的日子里，工厂出现泄漏并引发火灾。操作人员切断了进料，并试图释放工厂内的压力。然而，吹除阀却无法打开，原因是控制管路中有积水并已结冰。这导致火势持续时间超出预期，造成了更严重的损失。

起初，人们对水如何进入控制管路感到困惑。在两年前工厂改造时进行的危险与可操作性分析 (HAZOP) 中，团队曾被问及水是否可能进入控制管路，他们当时回答「不可能」。

在停车检修期间，有时需要操作这些阀门，但此时没有高压气体可用。维修团队成员被要求操作阀门，但没有得到具体的操作指导。他们便使用水和液压泵来操作阀门。然而，包括操作工工会代表在内的 HAZOP 分析团队成员都不知道阀门曾以这种方式被操作过。

这个案例说明，HAZOP 分析的效果取决于参与人员的知识和经验。如果他们不了解实际操作情况，HAZOP 就无法识别出潜在的危险。
...
尽管被认为是最佳可用工具之一，Kletz 还是指出了 HAZOP（危险与可操作性分析）存在一些不足 [2]。

专栏 1.29

HAZOP 无法检测出设计中的所有缺陷。特别是，它难以发现布局方面的问题。此外，对于那些穿过或靠近某个装置，但携带该装置未使用物料的管线，HAZOP 可能会忽视其泄漏带来的危险。为了解决这个问题，可以引入「穿过」或「附近」等额外的分析关键词。

HAZOP 分析基于一个假设，即施工和运行过程都严格遵循设计要求。然而，如果使用了错误的建造材料，或者设备未按计划进行测试，就可能引发问题。

因此，HAZOP 团队通常会特别关注那些需要采取特殊措施的情况，以确保使用正确的材料或进行必要的测试。他们还可能质疑某些设备的合理性，比如安装在跳闸阀周围的旁路系统，或者位于安全阀下方的隔离阀。
...
1.3.1 HAZOP 实施中的常见问题

1.3.1.1 将 HAZOP 误用为设计审查

Kletz 发现了一个普遍存在的问题：设计师往往过分依赖 HAZOP，认为它能解决设计过程中遇到的所有问题 [2]。
...
1.3.1.2 危害与可操作性分析（HAZOP）完成时间过晚一个常见问题是 HAZOP 在项目中进行得太晚。这导致人们不愿参与，因为他们明白：即使发现设计问题，此时再进行修正也将非常困难且成本高昂。这种情况通常出现在项目管理不善或未充分认识 HAZOP 价值的情况下。虽然 HAZOP 是必须完成的关键 deliverable，但常常被拖延到最后一刻才开始进行。

HAZOP 进行得太晚的问题在于，改进建议往往局限于增加额外的安全保护系统，而非实施本质上更安全的解决方案（如从设计层面消除潜在危险）。结果是，原有的危害依然存在，而由于添加了众多设备，反而可能增加了工厂的复杂度 [19]。

1.3.1.3 简化流程

HAZOP 面临的主要挑战之一是所需资源量大。因此，人们往往会尝试调整方法以减轻这一负担。

Kletz 曾举例说明这种做法 [2]。

Text Box 1.32

有一家公司试图通过仅关注设计规范和标准的偏差来简化危害与可操作性分析（HAZOP）。如果项目中几乎没有创新，这种做法可能是可以接受的。但如果存在创新 —— 而通常都会有一些创新 —— 现有的规范可能无法涵盖新出现的情况。举个例子，有一次一台液压起重机试图吊起一个对于完全伸展的吊臂来说过重的负载，结果整个起重机倒在了工厂上，但没有任何警报响起。这台起重机装配了当前规范要求的所有警报器，而且它们都处于正常工作状态。然而，这些规范最初是为机械式起重机制定的。液压起重机多了一个可变因素 —— 吊臂的长度可以改变 —— 因此需要增加一个额外的警报，但当时没有人意识到这一点。

有些人试图通过减少在会议或研讨会上进行的分析来简化 HAZOP 流程。他们为此辩解说，他们的设计师在设计和 HAZOP 方面都非常有能力，因此在工作过程中已经完成了彻底的分析。如果将这种观点推到极端，可能会导致认为根本不需要进行 HAZOP。这个问题曾被 Kletz 讨论过 [2]。

插文 1.33

"我们不需要 HAZOP。我们雇佣的都是优秀人才，我们相信他们的知识和经验。"

危害与可操作性分析（HAZOP）并不能替代知识和经验。它不是一个简单的自动化工具，不能仅靠输入线路图就生成修改清单。相反，HAZOP 是一种系统化和协调的方法，用于充分利用团队的知识和经验。由于现代设计变得越来越复杂，团队成员如果没有这种思维辅助工具，就难以有效地运用他们的知识和经验。因此，如果团队缺乏必要的知识和经验，即使进行 HAZOP 分析也难以产生有价值的结果。

有时候，即使是「优秀的人才」也可能陷入单独工作的困境。正如 Pegram 所言："当各个学科独立工作时，一个领域解决的问题可能会成为另一个领域的麻烦」，他还指出，"从单一角度看似成本较低的工程解决方案，最终可能并不是整体上最经济的选择。」HAZOP 的优势在于它能确保来自不同职能部门的人员共同系统地考虑潜在危险和操作问题。实践经验表明，当各个功能部门单独工作时，往往会忽视启动、关闭以及其他异常情况的考虑。

本节讨论的是一个普遍存在的误解：认为优秀的系统可以取代优秀的人才。然而，系统的作用仅限于确保人们的知识和经验得到系统性的应用，从而降低遗漏重要信息的可能性。如果参与者缺乏必要的知识、经验或投入，那么像危险与可操作性研究（HAZOP，Hazard and Operability Study）这样的系统就会沦为空壳。人们可能会按部就班地完成流程，但产出的质量必然低劣。反之，即使是优秀的人才，如果没有系统的辅助，也难以充分发挥其潜力。但若参与者本身就缺乏必要的知识和经验，再好的系统也无法产生有价值的成果。

有趣的是，让知识渊博、经验丰富的专家参与 HAZOP 也可能带来意想不到的问题。这些专家可能会基于自身经历，convincingly 地论证某些问题并不需要太多关注，因为他们曾亲眼目睹类似情况，而结果并不严重。这种经验虽然有助于保持 HAZOP 进程的流畅性，但也可能导致某些问题被忽视，尤其是那些发生频率低但后果严重的事件。另一种可能出现的情况是，如果团队过度关注一两个主要危害，他们可能会开始认为其他相对较小的危害不值得讨论。然而，我们知道大多数事故并非由单一的重大问题引起，而是多个相对较小的问题叠加的结果。

1.3.2 安全防护不当

在进行危险与可操作性分析（HAZOP）时，人们很容易将一系列现有项目识别为保障措施，或提出额外的措施，而不考虑这些措施在实际操作中能够合理达到的效果。例如：

- 现场操作人员从未检查的现场仪表，这些仪表实际上根本不能被视为保障措施；

- 由于故障发生速度太快，无法给操作员留出足够时间有效阻止偏差的警报。此外，还包括那些过于笼统、频繁触发，或在所考虑的情景中会成为警报洪流一部分的警报；

- 无法保证是为所研究情况而设计的压力泄放系统；以及

- 操作程序，特别是在导致某种情景发生的原因本身就是人为错误的情况下。

1.3.2.1 过度依赖人员

与生活中的所有事物一样，对危害与可操作性分析（HAZOP）采取平衡的方法是必要的，特别是在决定哪些安全措施是必需的，以及哪些因不能带来额外收益而可以省略时。在做出这些决定时，重要的是要认识到系统操作人员的能力，但同时也要对他们的能力和可靠性保持客观态度。如果有经验丰富的操作员参与这个过程，这可能会特别困难，因为如果他们对自己的职责产生了过度的保护意识，他们可能很难相信自己或同事会犯下导致正在讨论的异常情况的错误。这通常反映了一个更广泛的组织文化问题，源于操作员在事故和其他问题发生时被不公平地归咎。Kletz 指出，必须充分考虑人为错误的因素。

表 1.34

在考虑可能导致异常情况的原因时，也应该始终将人为错误纳入考虑范围。例如，流量为零可能是由于某人没有打开阀门造成的。这可能由多个原因引起：

操作员可能不知道应该打开阀门，也就是说，其操作意图本身就是错误的。

在这种情况下，我们可能需要改进培训和操作指令，或者简化工作流程。

操作员可能认为当时没有必要打开阀门，或者其他任务更为紧急。针对这种情况，我们可能需要解释操作指令背后的原因，并确保这些指令得到严格执行。

阀门可能过于坚硬或位置难以触及。更有可能的是，操作者可能出现了一时的疏忽或注意力分散，虽然操作意图正确，但未能完成预期目标。人人都难免会有疏忽和走神的时候，这些是无法完全避免的，尽管我们可以采取各种措施来降低其发生的可能性。如果潜在后果十分严重，我们应该通过优化设计或改进工作方法来消除或减少错误发生的机会（或者保护人员免受不良后果的影响，或使系统能够从错误中恢复）。

在进行危害与可操作性分析（HAZOP）时，由于缺乏对人为因素的深入理解，常常会出现两个问题：

- 错误地认为只要是有能力的人员严格遵守操作程序就不会犯错，从而忽视了可能的偏差；

- 将操作程序笼统地视为有效的安全保障，但未能具体说明需要什么样的程序，以及这些程序应包含哪些内容才能确保其有效性。

尽管在 HAZOP 分析中可以在一定程度上考虑操作程序的作用，但分析的重点必须始终放在寻找设计层面的解决方案上。这是因为 HAZOP 团队无法掌控系统实际运行时会采用什么样的程序，也无法决定负责操作、维护和管理系统的人员的实际能力水平。

1.3.2.2 警惕对自动化系统的过度乐观

危险与可操作性分析（HAZOP）的一个非常常见的建议是在设计中增加额外的安全联锁或安全仪表系统（Safety Instrumented Systems，SIS）。Kletz 指出，HAZOP 团队之所以会提出这样的建议，是因为他们认为这是一种既便宜又有效的解决方案。然而，他强调事实并非如此简单。

虽然很多问题确实可以通过这种方式解决，但成本往往比我们想象的要高得多。即使考虑了价格优惠，测试和维护一个额外的安全联锁系统的成本也差不多等于购买和安装的费用。换句话说，安全联锁系统的实际成本可能是我们预期的两倍。更重要的是，确保这些测试得到切实执行还需要大量的管理工作。一旦管理层对此失去兴趣，相关的操作程序可能会比金属老化得更快，甚至会悄无声息地消失。因此，与其安装额外的安全联锁系统，不如在设计的早期阶段就进行系统性的研究，找出避免潜在危险的方法。

另一个常见的误区是人们倾向于认为自动化系统是万无一失的。Kletz 指出这种想法是错误的。

他强调，在分析可能的异常情况时，我们必须考虑到自动设备故障的可能性。例如，如果发现没有物料流动，原因可能是安全联锁系统触发了，或者是控制器没能正常打开阀门，又或者是在错误的时间关闭了阀门。

事实上，Kletz 建议，任何用于控制的软件都应该接受 HAZOP 分析 [2]。

图 1.37：HAZOP 分析中的常见误区

相反，对给予计算机的指令（即应用软件）进行危险与可操作性分析（HAZOP，Hazard and Operability Study）是至关重要的。这是因为与人类操作员不同，计算机无法识别和质疑那些模糊不清、缺乏精确性或完全错误的指令。此外，软件工程师很少同时具备化学工程背景，因此很容易误解化学工程师的需求。

另一个非常普遍的问题是，在 HAZOP 分析中经常将报警系统指定为安全措施。这在一定程度上导致了包括过程工业在内的许多行业出现了一个非常严重的问题：报警系统未能实现其关键目标。造成这种情况的原因是操作员常常收到大量不需要响应的干扰报警，而且每当系统出现故障或异常时，他们就会被铺天盖地的报警信息淹没，根本无法理解和处理。许多公司已经意识到这个问题，并正在努力解决它 [9]。解决方案通常包括删除或修改某些报警设置。然而，这可能意味着在 HAZOP 分析期间做出的假设不再有效，甚至可能质疑研究中所假定的安全措施的基本依据。

1.3.2.3 忽视连锁反应

为了发挥实效，危害与可操作性分析（HAZOP）不仅要识别问题，还应该考虑潜在的解决方案。然而，这其中需要取得平衡，因为通常没有足够的时间来全面评估任何提议的设计变更。Kletz 指出，未能考虑潜在的连锁效应是一个特别值得关注的问题 [2]。

插文 1.38

在 HAZOP 过程中，如果对设计（或操作条件）进行更改，可能会影响工厂的其他部分，包括已经分析过的区域。

举个例子，在一次 HAZOP 中，团队决定为一个热交换器连接一个备用冷却水源。原有的水源很干净，但备用水源却被污染了。因此，团队不得不更改热交换器和连接管道所使用的钢材等级。他们还必须考虑原有管道中可能出现的反向流动问题。

行动措施的表述方式可能会显著影响其实施效果。如果表述过于具体，可能导致在没有充分评估预期和非预期后果的情况下就匆忙实施。然而，笼统的「考虑」某项变更的行动措施往往会被人轻易地搪塞过去，声称已经考虑过并决定不需要做任何改变。

1.3.3 缺乏后续跟进显然，HAZOP 本身并不能提高安全性。真正起作用的是由此产生的具体行动。

对于来自 HAZOP（危害与可操作性分析，Hazard and Operability Study）或任何其他研究的建议行动被拒绝是可以理解的，但不应该被完全忽视。在大多数情况下，拒绝某项建议时应该提供合理的解释，通常是因为找到了一个更有效的替代方案。即使决定拒绝某项行动而不采取任何措施，也应该清楚地解释为什么认为当前风险是可以接受的，并且已经达到了合理可行尽量降低（ALARP，As Low As Reasonably Practicable）的程度。正如文献 [15] 中所说："对一个公司而言，进行了 HAZOP 分析却忽视其建议，比完全不进行 HAZOP 分析更加有害。"

有一种做法需要特别警惕，那就是将建议的行动推迟到项目的后期阶段 [15]。这种做法往往会导致原本的行动建议被草率地关闭，因为当前的项目参与者认为他们暂时无法实施解决方案。对于这种情况，只有在将该行动转移到另一个跟踪系统（比如风险登记册，用于记录和跟踪项目风险的工具）以确保日后得到适当关注时，才应该关闭该行动项。否则，应该将该行动标记为「延期」，并详细说明在什么时候需要重新审视和解决这个问题。

将 HAZOP（危险与可操作性分析）作为强制性活动的一个问题，无论是作为项目的一部分还是作为计划中的回顾性审查，是它可能被视为一种「例行公事」，人们认为只需快速轻松地完成，目的是发现尽可能少的问题。为了部分避免这种情况，可以确保制定清晰的程序来描述如何执行 HAZOP，但仍需强调其重要性。让一位高级经理负责 HAZOP 报告的最终「签字」可以提高他们对此事的重视程度 [14]。在这方面，管理者的责任是确认所有行动都已完成或正式接受为例外情况，并且 HAZOP 过程已按公司指南由合格人员执行。

1.3.4 HAZOP 可能无法避免的事故

Kletz 明确指出，HAZOP 无法预防所有事故 [2]。

文本框 1.39

HAZOP 无法预防所有事故，至少有三个原因：(a）作为人类，我们难免会有疏漏，无法预见所有可能出错的地方；(b）人们的行为并不总是符合我们的预期；(c）由于 HAZOP 的特性，它无法预防某些机械事故，例如人们碰撞到位置不当的设备。

Kletz 强调，HAZOP 团队必须假设系统将被正确操作和维护 [2]。

文本框 1.40

他们假设总体管理水平是 competent 的，工厂的运营和维护将符合设计团队的设想，并遵循良好的管理和工程实践。特别是，他们假设保护系统将定期进行测试，并在必要时得到及时修复。

如果这些假设不成立，那么危害和可操作性研究（HAZOP）就是在浪费时间。如果没有人愿意采取任何行动，那么识别危险或估计其概率就毫无意义；如果没有人会使用或维护它们，那么安装跳闸保护和警报系统也是徒劳的。与其花时间进行 HAZOP，不如将这些时间用于将员工和管理层的安全意识提升到标准水平。

1.3.4.1 Piper Alpha（派珀阿尔法平台)

Piper Alpha 悲剧导致 167 人在一座海上石油生产平台起火时丧生，这是因为在压力安全阀（PSV）被拆除进行维护的位置发生了碳氢化合物（主要成分是石油和天然气）泄漏。虽然 HAZOP 会将 PSV 识别为防止管道过压的安全保护措施，但它会假设只要存在碳氢化合物，PSV 就应该始终处于正常工作状态。

从最基本的角度来看，Piper Alpha 事故的发生是由于管道接头处泄漏了碳氢化合物（hydrocarbon）。具体是哪个接头泄漏并不是最关键的。事故的根本原因在于维护和隔离管理的严重缺陷。虽然危险与可操作性分析（HAZOP，Hazard and Operability Study）可以在宏观层面强调这些流程的重要性，但它无法涵盖所有具体情况。

然而，如果当时在 Piper Alpha 平台上进行了 HAZOP 分析，可能就会发现有必要采取行动，考虑如何降低蒸气云爆炸的潜在危害。正如 Kletz 所解释的 [12]：

【文本框 1.1】

凝析油区域的爆炸产生的超压摧毁了将其与原油提取设备区域隔开的防火墙。防护屏障被突破后，立即引发了大规模的油火。这场火灾以及随之而来的浓烟笼罩了整个平台，严重阻碍了人员的逃生尝试。

显然，在石油平台的设计阶段，工程师们已经预见到凝析油区域可能因泄漏而引发火灾。这种情况发生的概率被认为相当高，以至于他们在装置的各个区域之间安装了防火隔离墙。考虑到这种风险评估，令人费解的是，他们似乎认为凝析油区域发生爆炸的可能性反而更低。然而，事实上，Piper Alpha 石油平台在凝析油区域的两侧都没有安装防爆墙。

更为严重的问题是，在设计阶段，工程师们没有对平台上可能发生的主要危险进行系统性的评估，在必要时还应包括定量分析。他们甚至没有对设计方案进行危害和可操作性研究（HAZOP，一种系统性的安全分析方法）。

1.3.4.2 邦斯菲尔德事故

Buncefield 石油储存库发生火灾和爆炸的原因是一个用于储存汽油的储罐被过度填充。这起事故源于操作失误，而本应在储罐液位达到特定高度时触发警报的液位仪表却未能正常工作。通过危害和可操作性研究（HAZOP）本可以将高液位警报识别为防止此类事故的安全保障措施，这本应是研究的合理结果。然而，HAZOP 团队必须假设，一旦安装，这些仪器将会得到妥善维护并保持在可操作状态。尽管该研究可以用来强调安全保障措施的重要性，但这种强调也只能停留在一般层面上，因为它适用于与警报和自动停机系统相关的广泛仪器设备。

其他类型的研究，如保护层分析（LOPA），可能会对那些仅能触发警报而没有自动停机功能的仪器的充分性提供更深入的见解。但这种分析仍然基于所有设备都得到适当维护的假设。

另一种方法是进行固有安全设计研究。这种研究可能已经确定了消除巨大蒸汽云爆炸潜在风险的方案。它着眼于风险减少技术层次结构中更高级的替代方案，而不是仅仅依赖于额外的保护层（如液位开关和阀门等容易发生故障的设备）。本书的固有安全章节中包含了对 Buncefield 事故中固有安全设计考虑的讨论。

1.4 实际应用本章旨在分享多角度的 HAZOP（危险与可操作性分析）观点，这些观点既基于 Kletz 的著作，也来源于其他人的近期经验。本节最后将为那些可能组织或参与 HAZOP 研究的人提供一些额外的实用指导。

1.4.1 HAZOP 的实施方法与时机开展 HAZOP 分析需要充分的计划和组织。Kletz 为我们提供了一些富有洞察力的观点，帮助我们理解在这个过程中需要解决的关键问题。

1.4.1.1 确保合适人员的参与

Kletz 强调，HAZOP 并非万能良药，其效果完全取决于参与人员的投入。

文本框 1.42

技术工具本身并不能发现危险；真正发现危险的是人。技术只是辅助工具，帮助人们更好地完成这项工作。HAZOP 的作用在于帮助人们运用自身的知识和经验，而这些知识和经验大多来自于学习和铭记过去的教训。

关于 HAZOP 团队在整个研究过程中是否应保持不变，Kletz 提出了自己的看法。他认为，特别是在较长期的研究中，团队成员可以适当变动。这样做有两个好处：一是可以避免团队成员因长期参与同一项目而产生的疲劳感（即 HAZOP 疲劳）；二是让不同的人参与可以使对设计、危险和潜在操作问题的理解更加广泛地分布在团队中，从而带来更多收益。

Kletz 还指出，某些特定问题可能需要专家的专业意见，但这些专家不一定需要全程参与 HAZOP 分析。

文本框 1.43

相反，如果一个工业设施存在特定问题（比如严重的腐蚀问题），这些问题的专家应该加入团队。但不要让专家仅仅为了可能需要他们的建议而坐在会议室里。

专家可以随时待命。现在远程通讯和视频会议设备已经普及，这意味着专家们不必亲临现场也能提供意见。

1.4 实际应用

1.4.1.2 不要将工作全权交给承包商一个值得关注的问题是，在一些项目中，大部分设计工作是由第三方完成的。通常，完成危险与可操作性分析（HAZOP）是他们合同的一部分，这可能会让人觉得他们应该独自完成这项工作。但 Kletz 强调，这绝对不是一个好主意 [2]。

专栏 1.44

有些公司曾经对设计承包商说："我们这边人手紧张，而你们是专家，那么为什么不帮我们做 HAZOP 呢？"

客户应该与承包商一同参与项目，因为最终是客户要负责运营该工厂。危险与可操作性分析（HAZOP）能够帮助客户的员工理解各种设计特征的原理，并协助他们编写操作规程。即使客户的员工最初对特定工艺的具体问题了解不多，他们也能运用一般的化学工程和科学知识，以及日常经验来贡献自己的见解。Pegram 在另一个场合中指出："... 唯一有效的团队是真正承担问题的团队。因此，团队必须由那些负责实施研究结果的人员组成，而不是外部专家组。」HAZOP 分析后通常会提出一系列建议，包括程序的修改和设备的改进。虽然承包商负责后者，但客户则负责前者的实施。

1.4.1.3 如何开展 HAZOP

一些人认为他们不需要进行 HAZOP 分析，因为他们只处理小型项目，他们（错误地）认为 HAZOP 不适用于这些项目。这种想法不仅导致小型项目缺乏必要的安全审查，还可能使公司和人员在面对大型项目时措手不及。Kletz 对此观点进行了总结 [2]。

框 1.45

相反，我们应该从小处着手，先将危险与可操作性研究（HAZOP）应用于一两个项目，看看是否有效。如果效果不错，我们可以逐步扩大应用范围，直到对所有新建工厂和尽可能多的现有工厂都进行 HAZOP 分析。

1.4.1.4 工厂改造也需要进行 HAZOP 分析

Kletz 注意到，人们有时认为 HAZOP 只适用于新项目，因此在进行改造时往往不会使用。他对此表示担忧，并总结如下 [2]：

> 文本框 1.46
> 
> 一项未经充分考虑的改造可能会在随后的几个月里引发一系列连锁反应，甚至影响到工厂的偏远区域。
> 
> ——N. Pegram，《化学工程师》第 482 期，1990 年他进一步解释道 [2]：

> 文本框 1.47
> 
> 许多人认为 HAZOP 不适合小型改造，因为每次安装新阀门、增加采样点或提高操作温度时，都很难组织一个团队进行分析。然而，许多事故的发生正是由于改造带来了意想不到的负面影响。因此，即使不进行完整的 HAZOP 分析，在批准任何改造方案之前，也应该对其进行彻底的审查。

在进行改造时，不要忽视以下几点：

- 临时改造与永久改造同样重要；
- 新建工厂的启动改造和现有工厂的改造都需要注意；
- 不论改造成本高低，都应该谨慎对待；以及

- 对程序、生产材料或操作条件的修改，以及对设备的改动。

在任何变更管理（Management of Change）程序中，考虑进行危害与可操作性分析（HAZOP，Hazard and Operability Study）的要求应该是不可或缺的。应该明确指出在哪些情况下 HAZOP 是强制性的、推荐的或不需要的。

1.4.1.5 ' 合理可行 ' 原则的应用一个常见的担忧是，公司必须执行 HAZOP 分析所提出的所有建议，即使这些建议非常昂贵。然而，Kletz 指出这种观点并非总是正确的 [2]。

> 文本框 1.48
> 
> HAZOP 能够发现问题并提出解决方案。但如果解决这些问题的成本过高，而风险又不是完全不可接受的（用法律术语来说，如果采取措施不是「合理可行的"），我们就不必强制执行这些解决方案。然而，我们也不应该因为担心发现不想看到的问题就闭上眼睛视而不见。

显然，要证明风险已经降低到合理可行且实际可行的水平（ALARP，As Low As Reasonably Practicable），远不止是因为某项措施昂贵就简单地拒绝它。任何拒绝采取行动的决定都应该有详细记录的理由，主要关注为什么认为风险降低的程度不够。理想情况下，应该提出一个替代方案来解决 HAZOP 中提出的问题，这个方案应该更有效和 / 或更经济。

1.4.1.6 自动化 HAZOP

随着技术的发展，HAZOP 分析也在向自动化方向演进。下面我们将探讨自动化 HAZOP 的优势和挑战。

如今，用于记录 HAZOP（危险与可操作性分析）输入和输出的软件已经广泛普及，但研究人员希望在此基础上更进一步，实现 HAZOP 过程的自动化。多年来，许多研究团队一直在探索这个方向，但迄今为止还没有一个团队成功开发出可实际应用的系统。这可能是因为计算机难以模拟知识丰富、经验丰富的团队所具备的创造力和判断力。在这个领域，Kletz 曾参与相关研究并分享了他的见解 [2]。

文本框 1.49

计算机目前被广泛应用于记录结果和提醒团队关注常见的偏差原因。此外，一些旨在生成技术问题清单的程序也已经开发出来，但目前使用还不广泛。然而，计算机很难识别那些源于人与人之间互动（或缺乏互动）而产生的问题。

1.4.1.7 其他类型的 HAZOP

HAZOP 只是危险识别研究中众多方法之一。为了满足特定需求，研究人员开发了一些 HAZOP 的变体。以下是几种较为知名的变体：

- 批次 HAZOP：适用于批处理过程。在这种方法中，除了标准的引导词外，还增加了「过早」、「过晚」、「之前」、「之后」等时序相关的引导词；

- 人员或程序 HAZOP：这种方法主要关注人为错误，分析其可能造成的后果以及相应的控制措施；

- 软件或控制 HAZOP（CHAZOP)：专门用于识别软件系统中的潜在弱点和错误，提高软件的安全性和可靠性。

使用这些危害分析方法的研究可以采用与标准危害和可操作性分析（HAZOP）非常相似的方法，但需要具备相关专业知识的团队领导和 / 或团队成员。

1.4.2 过程 HAZOP 的具体注意事项

Kletz 为我们提供了在进行过程工业 HAZOP 时需要注意的许多具体事项。这些事项有助于我们更全面地识别和评估潜在的危害。

1.4.2.1 容器的 HAZOP

Kletz 建议在 HAZOP 过程中有时需要对容器进行特别考虑 [2]。

文本框 1.50

在分析完所有通向容器的管线后，我们会将引导词「OTHER THAN」（除此之外）应用于该容器。虽然对容器本身应用其他引导词并非必要，因为大多数问题应该在分析进出口管线时被发现，但为了减少遗漏的可能性，我们仍应将引导词应用于容器中进行的所有操作。例如，如果容器内发生沉降，我们会考虑是否可能出现无沉降、反向沉降（即混合）、沉降过多或过少的情况。同样的方法也适用于搅拌、加热、冷却和其他任何操作。

1.4.2.2 中间储存

Kletz 对固有安全的关注使他说了这样一句话："你应该长出白发，而不是依赖中间储存。」这句话形象地表达了他对减少中间储存的重视。尽管如此，中间储存通常是必需的，因此在 HAZOP 分析中应该得到特别关注 [2]。

1. 危害和可操作性（HAZOP）分析文本框 1.51

（注：原文中此处只有标题，没有具体内容。）

请特别关注中间存储设备。一般来说，除了装料和卸料，这些设备中不应该发生其他变化。但是，如果内容物停留的时间比平常长，可能会出现温度或成分的变化。

1.4.2.3 小型支线

Kletz 指出，危险与可操作性研究（HAZOP）可能会过于关注大型设备，这有时会导致看似不太重要的部分被忽视，比如小型支线 [2]。

文本框 1.52

不要忽视那些可能没有编号的小型支线。举个例子，一个储罐装有一个加料漏斗，用于添加稳定剂。在进行 HAZOP 时，显然应该考虑添加剂量过多、过少，使用错误添加剂或在错误时间添加等情况的影响。但如果团队只研究有编号的管线，这些情况可能会被忽略。（不过，他们可能会通过考虑容器内的操作而发现这一问题，这也是 HAZOP 经常给我们第二次发现问题机会的另一个例子。）

1.4.2.4 询问设计细节

Kletz 建议，HAZOP 是一个很好的机会来询问一系列设计细节，这些细节可能并不总是直接与工艺过程相关 [2]。

文本框 1.53

HAZOP 还提供了一个机会，让我们检查设计过程中是否考虑了一些具体细节。团队应该提出以下问题：

- 使用了哪些类型的密封垫？是否应该使用金属缠绕垫片？密封垫的种类是否已控制到最少？(我们使用的种类越多，误用的可能性就越大。)

- 螺母和螺栓的种类是否已控制到最少？

- 所使用的阀门是否属于那种位置一目了然的类型，比如升降杆阀？

- 如果使用了球阀或旋塞阀，它们的手柄是否有可能被安装在错误的位置？

- 在预计需要定期进行盲板操作（用于维护或防止污染）的接头处，是否安装了双面盲板？

- 通常，可达性问题会在设计的后期阶段考虑，即当工厂模型（实物或计算机模型）可用时。但是危险与可操作性研究（HAZOP）团队应该注意任何需要特别关注的点。例如，那些需要频繁操作或在紧急情况下操作的阀门应该易于接近。

1.4.3 总结

Kletz 分享了关于 HAZOP 潜在威力的以下观点 [2]。

插文 1.54

Carling 描述了在他的公司中使用危险与可操作性分析（HAZOP，Hazard and Operability Analysis）的效果。这些收益远远超出了一份简单的安全工厂建议清单。团队成员之间的互动带来了个人和部门态度的深刻变化。员工开始主动寻找同事讨论 proposed 变更可能带来的后果，问题被更公开地讨论，部门间的竞争和壁垒逐渐消除。大家更深刻地认识到了独立工作的风险，以及草率和仓促行动可能造成的严重后果。知识、想法和经验得到了更充分的分享，这既有利于个人，也有利于公司的发展。

Carling 的公司是在经历了几次严重事故后才采用 HAZOP 的。Buzzelli 写道："对于一个一直以技术安全成就为傲的行业来说，不得不承认我们大多数重要的安全改进都是在响应工厂事故后才开发的，这实在是一个发人深省的事实。"

但事情不必如此。HAZOP 就像是为我们的航船提供了一盏前照灯，让我们能够在危险真正造成损害之前就发现它们。

一份回顾了 40 种不同危险识别方法的出版物总结了 HAZOP 的优点如下：

- 这是一种系统性和全面的技术。有一个详细的执行计划，它能系统地将指导词（guidewords）和参数（parameters）应用于过程中的所有管道和设备。

- 它分析失效可能带来的影响。评估团队应当仔细考虑已识别出的偏差可能导致的后果。这有助于制定建议，以最小化或减轻潜在的危害。

同一研究还总结了危害与可操作性分析（HAZOP，Hazard and Operability Study）方法的以下缺点：

- 耗时且成本高。大多数工厂都包含大量的管道和设备，每一个都需要通过应用各种引导词和参数进行详细检查。

- 需要详细的设计图纸才能进行完整的研究。为了全面执行 HAZOP 分析，生产过程必须设计得足够详细，包括所有管道和设备的运行条件以及控制仪器的具体信息。

- 对于特殊危险需要额外的引导词。一般引导词可能无法涵盖某些特定的危险，因此需要引入额外的专门词语（例如核工业中的「辐射"）。

- 需要经验丰富的专家。需要有丰富经验的团队成员来识别偏差的所有可能原因和后果，并提出切实可行的建议。

- 只关注单一事件导致的偏差。这种方法只能研究与单一偏差相关的危险。由两个或更多独立偏差共同引起的危险无法通过该技术识别。

另一项研究 [13] 表明，危害和可操作性分析（HAZOP，Hazard and Operability Analysis）在确保系统的「概念设计、配置和规格说明」方面非常有效，这有助于改进制造、调试、操作和维护工厂的各项规格。除此之外，HAZOP 还带来了许多额外的好处，包括可以作为系统生命周期内决策的重要参考，特别是在进行系统修改时。此外，它还能以一种特殊的方式教育相关人员，有助于改进未来的设计并消除不同专业领域之间的潜在冲突。

在设计阶段进行的 HAZOP 分析为我们提供了机会，既可以避免那些发生概率高但后果较轻的事件，又能确保针对那些虽然发生概率低但后果严重的事件采取适当的防护措施。而在系统运行一段时间后回过头来进行的 HAZOP 分析则具有经验优势，能够让我们更深入地了解潜在问题，以及这些问题演变为事件甚至可能酿成事故的可能性 [13]。

HAZOP 是少数几种拥有正式指导方法的分析技术之一。然而，我们不应将其视为识别危险的唯一选择。实际上，选择何种方法应该根据具体应用场景和可用资源来决定 [9]。
